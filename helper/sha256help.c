...
unsigned int z[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
...
{
	n->S1 = RR(n->e, 6) ^ RR(n->e, 11) ^ RR(n->e, 25);
	n->ch = (n->e & n->f) ^ ((~n->e) & n->g);
	n->temp1 = n->h + n->S1 + n->ch + z[i] + w[i];
	n->S0 = RR(n->a, 2) ^ RR(n->a, 13) ^ RR(n->a, 22);
